/*----------------------------------------------------------------------------
 * Jasmine code generator, a tool to build web crud application,with spring-
 * boot, mybatis, mysql,swagger,spring-security.
 * Generated at 2020-7-31 9:09:23 PM 
 * All rights reserved by fuanlei(email:767550758@qq.com) since 2019
 *---------------------------------------------------------------------------*/
package com.jasmine.crawler.controller;

import com.github.pagehelper.Page;
import com.jasmine.crawler.pojo.entity.BindRecord;
import com.jasmine.crawler.pojo.req.GetBindRecordPageReq;
import com.jasmine.crawler.pojo.resp.PageResult;
import com.jasmine.crawler.pojo.resp.R;
import com.jasmine.crawler.service.BindRecordService;
import io.swagger.annotations.Api;
import io.swagger.annotations.ApiOperation;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.validation.annotation.Validated;
import org.springframework.web.bind.annotation.*;

@Api(tags = "BindRecord")
@RestController
public class BindRecordController extends ControllerBase {

    @Autowired
    private BindRecordService bindRecordService;

    @ApiOperation("get bindRecord page")
    @GetMapping(path = "/bindRecord/page")
    public R<PageResult<BindRecord>> getPage(@Validated @ModelAttribute GetBindRecordPageReq req) {
        PageResult<BindRecord> result = bindRecordService.getPage(req);
        return responseData(result);
    }

}

