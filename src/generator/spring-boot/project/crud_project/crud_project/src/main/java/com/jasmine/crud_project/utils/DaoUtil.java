...
import com.jasmine.crud_project.pojo.req.PageReq;
import com.jasmine.crud_project.pojo.req.PageTimeReq;
...
public class DaoUtil {
    ...
    public static <T> T queryOne(
        ...
        PageResult<T> result = queryNormalPage(mapper, pageReq, targetClazz, null, null);
        return CollectionUtil.getFirst(result.getList());
    }
     ...
     * @return
     */
    public static <T> boolean update(Mapper<T> mapper, Object source, Class<T> targetClazz, String... conditions) {
        T instance = BeanUtils.instantiateClass(targetClazz);
        setEmptyStringFieldToNull(source);
        BeanUtils.copyProperties(source, instance);
        ReflectUtils.setPropertiesToNull(instance, conditions);
        Example example = createUpdateExample(source, targetClazz, conditions);
        return mapper.updateByExampleSelective(instance, example) > 0;
    }

    private static <T> Example createUpdateExample(Object source, Class<T> targetClazz, String... conditions) {
        T instance = ReflectUtils.instantiateWithSpecificProperties(targetClazz, source, conditions);
        Example example = new Example(targetClazz);
        Example.Criteria criteria = example.createCriteria();
        setEqualProperties(instance, targetClazz, criteria);
        return example;
    }
