/*----------------------------------------------------------------------------
 * Jasmine code generator, a tool to build web crud application,with spring-
 * boot, mybatis, mysql,swagger,spring-security.
 * Generated at 6/27/2021, 1:59:23 PM 
 * All rights reserved by fal(email:767550758@qq.com) since 2019
 *---------------------------------------------------------------------------*/
const { all } = require("./../db/all")
const { ConfigItemBuilder } = require("D:/project/code-generator/src/generator/spring-boot/builders/ConfigItemBuilder")


exports.pPageConfig = {
        table: all.pPage,
        name: "PPage",
                items: [
                        // add
                        // createTime excluded 
                        // description : validate --- @NotEmpty  
                        // id excluded 
                        // lastBackupTime excluded 
                        // menuId : validate --- @NotNull  
                        // name : validate --- @NotEmpty  
                        // pageType : validate --- @NotNull
                        new ConfigItemBuilder()
                                .type("insert")
                                .id("add")
                                .alias("t")
                                .includes(collection => {
                                        collection.includes(all.pPage.columnsArray)
                                                  .excludes("id")
                                                  .excludes(["createTime","id","lastBackupTime",])
                                })
                                .request(request => {
                                        request.path("")
                                               .description("add pPage");
                                })

                                .req(req => {
                                        req.doCreate()
                                           .from("@RequestBody")
                                           .excludes("id")

                                           .validate("description","@NotEmpty")
                                           .validate("menuId","@NotNull")
                                           .validate("name","@NotEmpty")
                                           .validate("pageType","@NotNull")
                                })
                                .build(),

                        // deleteById
                        new ConfigItemBuilder()
                                .type("delete")
                                .id("deleteById")
                                .alias("t")
                                .conditions(collection => {
                                        collection.includes("id")
                                                  .require("id")
                                })
                                .request(request => {
                                        request.path("{id}")
                                               .description("delete single pPage");;
                                })
                                .req(req => {
                                        req.name("id")
                                           .type("Integer")
                                           .from("@PathVariable");
                                })

                                .build(),

                              // updateById
                        // createTime : excluded 
                        // description : validate --- @NotEmpty  
                        // id : validate --- @NotNull  
                        // lastBackupTime : validate --- @NotNull  
                        // menuId : validate --- @NotNull  
                        // name : validate --- @NotEmpty  
                        // pageType : validate --- @NotNull
                        new ConfigItemBuilder()
                                .type("update")
                                .id("update")
                                .alias("t")
                                .includes(collection => {
                                        collection.includes(all.pPage.columnsArray)
                                                  .excludes(["createTime",])

                                })
                                .conditions(collection => {
                                        collection.require("id")
                                })
                                .request(request => {
                                        request.path("")
                                               .description("update single pPage");;
                                })
                
                                .req(req => {
                                        req.doCreate()
                                           .from("@RequestBody")
                                           .validate("description","@NotEmpty")
                                           .validate("id","@NotNull")
                                           .validate("lastBackupTime","@NotNull")
                                           .validate("menuId","@NotNull")
                                           .validate("name","@NotEmpty")
                                           .validate("pageType","@NotNull")
                                           .name("UpdatePPageReq")                                                
                                })
                                .build(),

                        // getById
                        new ConfigItemBuilder()
                                .type("select")
                                .id("getById")
                                .alias("t")
                                .includes(collection=>{
                                        collection.includes(all.pPage.columnsArray)
                                })
                                .conditions(collection =>{
                                        collection.includes("id")
                                                  .require("id")
                                })
                                .request(request => {
                                        request.path("{id}")
                                                .description("get single pPage");;
                                })
                                .req(req => {
                                        req.name("id")
                                           .type("Integer")
                                           .from("@PathVariable");
                                })
                                .resp(resp => {
                                        resp.single();
                                })

                                .build(),


                        // get detail page
                        // createTime : excluded 
                        // description : excluded 
                        // lastBackupTime : expression --- timeRange
                        // menuId : excluded 
                        // name : expression --- like
                        // pageType : excluded
                         new ConfigItemBuilder()
                         .type("select")
                         .id("getDetailPage")
                         .alias("t")
                         .includes(collection=>{
                                 collection.includes(all.pPage.columnsArray)
                         })
                         .conditions(collection=>{
                                  collection.includes(all.pPage.columnsArray)
                                            .excludes("id")
                                                  .excludes(["createTime","description","menuId","pageType",])
                                                  .exp("lastBackupTime","timeRange")
                                                  .exp("name","like")
                          })
                                .join(join=>{
                                            join.table(all.pMenu)
                                                .includes(collection=>{
                                                        collection.includes("name")
                                                                   .alias("name","pMenuName")
                                                })
                                                .type("left")
                                                .alias("t1")
                                                .joinCondition(" t.menu_id = t1.id")
                                                .build()
                                })
                                
                         .request(request => {
                                         request.path("detail/page")
                                                .description("get pPage page with additional details");;
                        })
                         .req(req => {
                                  req.doCreate()
                                 .excludes("id")

                                 .name("GetPPagePageReq")
                         })
                   
                         .resp(resp=>{
                                 resp.doCreate()
                                     .name("PPageDetailResp")
                                     .build()
                         })
                         .build(),

                        
 ]
}
