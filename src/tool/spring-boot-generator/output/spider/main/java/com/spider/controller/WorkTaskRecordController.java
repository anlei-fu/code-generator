...
public class WorkTaskRecordController {
    ...
    @Autowired
    private WorkTaskRecordService service;

    @PostMapping("/WorkTaskRecord")
    @ApiOperation("新增worker 运行记录")
    public R add(@Validated @ModelAttribute WorkTaskRecordDao dao) {
       return R.add( () -> service.add(dao));
    }

    @DeleteMapping("/WorkTaskRecord/{id}")
    @ApiOperation("通过id删除worker 运行记录")
    public R deleteById(@PathVariable Integer id) {
      return R.deleteById(id, () -> service.deleteById(id));
    }
  
    @PutMapping("/WorkTaskRecord/{id}")
    @ApiOperation("通过id更新worker 运行记录")
    public R updateById(@PathVariable Integer id, @Validated @ModelAttribute WorkTaskRecordDao dao) {
        return R.updateById(id, () -> service.updateById(id, dao));
    }

    @GetMapping("/WorkTaskRecord/{id}")
    @ApiOperation("通过id更新worker 运行记录")
