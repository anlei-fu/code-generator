create or replace procedure SP_CITY_ORDER_REPORT1(v_start_time  varchar2, --起始时间
                                                 v_end_time    varchar2, --结束时间
                                                 v_time_type   number, --时间类型 1 --天 2 --月
                                                 v_province_no varchar2, --省份(单个)
  ...
  begin
    open v_out_data for
      with res as(
      select
         to_char(r.order_date,l_date_format) order_date, --订单日期
       ...
       from(select
            ...
            where
                 ...
                 and (v_province_no is null or ext.contact_province in(select * from table(f_string_split(v_province_no))))
            ...
            group by
               ...
               ext.contact_city
            order by trunc(t.create_time,l_date_trunc_pattern) desc , ext.contact_province,ext.contact_city
           )r
       )
