create or replace procedure SP_CITY_FLOW_REPORT1(v_start_time  varchar2, --起始时间
      ...
      with tmp_result as
       (
         ...
         from(
             ...
             left join dm_system_city city on t.city_no = city.city_no
             where
                   t.business_type = 4
               and (v_start_time is null or t.create_time >= to_date(v_start_time,'yyyy-MM-dd'))
             ...
             group by
               ...
               province.province_name,
               city.city_name
             order by  trunc(t.create_time,l_date_trunc_pattern) desc , province.province_name,city.city_name
            )r
         )
