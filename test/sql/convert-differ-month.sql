with r1 as
 (select trunc(t.report_time, 'mm') time,
   ...
   group by trunc(t.report_time, 'mm'), t1.promoter_id),
r2 as
 (select trunc(t.create_time, 'mm') time,
         sum(decode(t.order_status, 0, 1, 0)) valid,
         t.promoter_id
    from dm_order_main t
    where t.create_time >= trunc(sysdate-15)
          and t.create_time < trunc(sysdate+1)
   group by trunc(t.create_time, 'mm'), t.promoter_id),
r3 as
...
select r3.time,
       ...
       r3.convert,
       r3.valid,
       abs(r3.valid-r3.convert )differ
  from r3
  left join Put_Account_Info t1
