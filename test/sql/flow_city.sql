create or replace procedure SP_CITY_FLOW_REPORT(v_time       varchar2, --日期
                                                v_start_time varchar2, --起始时间
                                                v_end_time   varchar2, --结束时间
                                                v_time_type  number, --时间类型 0 --小时 1 --天 2 --月
                                                v_business   varchar2, --业务类型
                                                v_province   varchar2, --省份 汉字名字
  ...
  */
  l_time date := case
                   when v_time_type = 2 then
                    to_date(v_time, 'yyyy-mm')
                   else
                    to_date(v_time, 'yyyy-mm-dd')
                 end;
  l_start_time date := to_date(v_start_time, 'yyyy-mm-dd');
  l_end_time   date := to_date(v_end_time, 'yyyy-mm-dd');
 ...
 select count(1)
   ...
   where t.order_status = 0
     ...
     and (t.province_no in
             (select distinct dsp1.province_no
                 from dm_system_province dsp1
                where dsp1.province_name = v_province) or
             v_province is null);
  ...
  open v_out_data for
    ...
    select case
      ...
      from (select case
             ...
             where t.order_no in (select order_no from dm_order_main t
              where l_time = case
               when v_time_type = 2 then
                trunc(t.create_time, 'mm')
               else
         ...
         and t.create_time >= l_start_time
         ...
         and (t.province_no in
             ...
             group by (case
                               when v_time_type = 2 then
                                ...
                                trunc(t.create_time)
                             end, t.business_type, t.province_no, t.city_no)
      ...
      left join dm_system_city dsc
        on R.city_no = dsc.city_no
     ...
     select k.日期,
           ...
           decode(substr(k.不知情投诉率,1,1),'.','0'||k.不知情投诉率,k.不知情投诉率) as 不知情投诉率,
           decode(substr(k.营销宣传投诉率,1,1),'.','0'||k.营销宣传投诉率,k.营销宣传投诉率) as 营销宣传投诉率
           -- k.投诉率,
           --k.不知情投诉率,
           --k.营销宣传投诉率
            from resultOrder k
     union all select '合计',
         ...
         sum(k.订单量),
             sum(k.有效订单量),
              nvl(round(sum(k.有效订单量) / decode(sum(k.订单量), 0, null, sum(k.订单量)), 5) * 100,
               0) || '%',
               '1',
             sum(k.投诉量),
              /*nvl(round(sum(k.投诉量) / decode(sum(k.有效订单量), 0, null, sum(k.有效订单量)), 5) * 100,
               0) || '%',
                nvl(round(sum(k.不知情投诉量) / decode(sum(k.有效订单量), 0, null, sum(k.有效订单量)), 5) * 100,
               0) || '%',
                nvl(round(sum(k.营销宣传投诉量) / decode(sum(k.有效订单量), 0, null, sum(k.有效订单量)), 5) * 100,
               0) || '%'*/
                decode(substr(nvl(round(sum(k.投诉量) / decode(sum(k.有效订单量), 0, null, sum(k.有效订单量)), 5) * 100,
               0) || '%',1,1),'.','0'||nvl(round(sum(k.投诉量) / decode(sum(k.有效订单量), 0, null, sum(k.有效订单量)), 5) * 100,
               0) || '%',nvl(round(sum(k.投诉量) / decode(sum(k.有效订单量), 0, null, sum(k.有效订单量)), 5) * 100,
               0) || '%'),
               decode(substr(nvl(round(sum(k.不知情投诉量) / decode(sum(k.有效订单量), 0, null, sum(k.有效订单量)), 5) * 100,
               0) || '%',1,1),'.','0'||nvl(round(sum(k.不知情投诉量) / decode(sum(k.有效订单量), 0, null, sum(k.有效订单量)), 5) * 100,
               0) || '%',nvl(round(sum(k.不知情投诉量) / decode(sum(k.有效订单量), 0, null, sum(k.有效订单量)), 5) * 100,
               0) || '%'),
                decode(substr(nvl(round(sum(k.营销宣传投诉量) / decode(sum(k.有效订单量), 0, null, sum(k.有效订单量)), 5) * 100,
               0) || '%',1,1),'.','0'||nvl(round(sum(k.营销宣传投诉量) / decode(sum(k.有效订单量), 0, null, sum(k.有效订单量)), 5) * 100,
               0) || '%',nvl(round(sum(k.营销宣传投诉量) / decode(sum(k.有效订单量), 0, null, sum(k.有效订单量)), 5) * 100,
               0) || '%')
                from resultOrder k
     ;
