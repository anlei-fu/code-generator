create or replace procedure SP_CITY_ORDER_REPORT(v_time       varchar2, --日期
...
begin
  ...
  open v_out_data for
   ...
   and t.create_time >= l_start_time
    ...
    and
      ( ext.contact_province = v_province or v_province is null))
    select case
             when v_time_type = 2 then
           ...
           nvl(round(r.delivaryCount / decode(r.total, 0, null, r.total), 2)*100,
               0) || '%' as "发货率", --发货率
           nvl(r.activatecount, 0) "激活量", --激活量
           nvl(round(r.activatecount / decode(r.total, 0, null, r.total), 2)*100,
               0) || '%' as "激活率", --激活率
           nvl(round(r.activeT3 / decode(r.total, 0, null, r.total), 2)*100, 0) || '%' as "t+3激活率", --t+3激活率
           nvl(round(r.activeT7 / decode(r.total, 0, null, r.total), 2)*100, 0) || '%' as "t+7激活率", --t+7激活率
           nvl(round(r.activatecount / decode(r.valid, 0, null, r.valid), 2)*100,
